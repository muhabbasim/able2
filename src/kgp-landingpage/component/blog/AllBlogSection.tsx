...
const AllBlogSection = () => {
  ...
  return (
    <section
      className="tl-inner-blogs tl-inner-blogs-1 pt-120 pb-50"
      data-bg-color="var(--tl-1-bg-coffee)"
    >
                ...
                <div className="tl-1-blog-txt">
                  <div className="tl-1-blog-info">
                    <h6 className="tl-3-single-event-time">
                      <i className="fa-light fa-calendar-lines"></i> {item.date}
                    </h6>
                  </div>
                  <h4 className="tl-1-blog-title">
                    <Link to={`/blog/${item.slug}`}>{item.title}</Link>
                  </h4>
                  <Link to={`/blog/${item.slug}`} className="tl-1-blog-btn">
                    Continue Reading
